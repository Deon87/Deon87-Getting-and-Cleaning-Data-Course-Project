run_analysis <- function() {
  ...
  Mean_Std_ColNames <- c(grep("_mean_|_mean$|_std_|_std$|[^Freq]",names(Subject_XY_Test)))
 ...
 # Only mean and standard deviation measurements are needed
  ...
  Trained_MeanStd_ColNames<- c(grep("_mean_|_mean$|_std_|_std$|[^Freq]", names(Subject_XY_Train)))
  ...
  Tidy_Dataset <- aggregate(Complete_Datasets[,3:ncol(Complete_Datasets)], by=list(Subject=Complete_Datasets$Subject,Activity_Name=Complete_Datasets$Activity_Name), FUN=mean, na.rm=TRUE)
